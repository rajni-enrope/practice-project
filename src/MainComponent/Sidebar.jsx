...
const Sidebar = ({ tabs, activeTab, onTabClick, opensidebar, setopensidebar }) => {
  return (
    <nav   className={`h-screen text-white bg-gray-800 p-1 shadow-md duration-500
    fixed top-0 left-0 z-50
    ${opensidebar ? "w-60" : "w-10"} 
  ...
  style={{ zIndex: 1000 }}>
      ...
      <div className='flex justify-between'>
        ...
        <MdMenu size={30} className="cursor-pointer" onClick={() => setopensidebar(!opensidebar)} />
      </div>
      <div>
        <ul className="space-y-3">
          {tabs.map((tab) => (
            ...
            >
              <span
                className={`text-xl mr-[2px] rounded ${!opensidebar
                  ? activeTab === tab.label
                    ? 'bg-gray-600 '
                    : 'hover:bg-gray-700 hover:ring-1 hover:ring-gray-500'
                  : ''
