  ...
  `}
      ...
      <div>
        <ul className="space-y-3">
          {tabs.map((tab) => (<li key={tab.label} className={`flex relative items-center gap-2 mr-4 px-0.5 py-1 my-2 rounded-md cursor-pointer
              ${opensidebar && activeTab === tab.label ? 'bg-gray-600' : ''}
              ${opensidebar && activeTab !== tab.label ? 'hover:bg-gray-700' : ''} `} onClick={() => onTabClick(tab.label)} >
            <span
